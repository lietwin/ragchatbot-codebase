...
def test_vector_store(rag_system):
    ...
    try:
        ...
        print(f"Search result - Documents: {len(search_result.documents)}, Error: {search_result.error}")
...
def test_tool_manager(rag_system):
    ...
    try:
        ...
        for tool_def in tool_definitions:
            print(f"  - {tool_def.get('name', 'Unknown')}: {tool_def.get('description', 'No description')}")
        
        # Test tool execution
        result = rag_system.tool_manager.execute_tool("search_course_content", query="test query")
...
def test_ai_generator(rag_system):
    ...
    try:
        ...
        print(f"System prompt contains tools: {'search_course_content' in ai_gen.SYSTEM_PROMPT}")
...
def test_data_loading(rag_system):
    ...
    try:
        ...
        doc_files = [f for f in os.listdir(docs_path) if f.endswith(('.pdf', '.txt', '.docx'))]
